...
#Region "Sub/Funktionen"
    ...
    Function Auto_Update() As Boolean
        If Registry.CurrentUser.OpenSubKey("Software").OpenSubKey("RS_AP") Is Nothing Then
            Registry.CurrentUser.OpenSubKey("Software", True).CreateSubKey("RS_AP")
        End If
        ...
        If _strUpdate_Pfad = "" Then
            If MsgBox("Update Verzeichnis noch nicht definiert" & vbNewLine & "Wollen Sie dies auswählen?", MsgBoxStyle.YesNo, "RS_AP Auto Update") = vbYes Then
        ...
        If System.IO.File.Exists(_strApp_Pfad + "\RS_AP.Update.exe") Then
            _strVersion_Update_Lokal = FileVersionInfo.GetVersionInfo(_strApp_Pfad + "\RS_AP.Update.exe").FileVersion.ToString
        End If

        If System.IO.File.Exists(_strUpdate_Pfad + "\RS_AP.Update.exe") Then
            _strVersion_Update_Remote = FileVersionInfo.GetVersionInfo(_strUpdate_Pfad + "\RS_AP.Update.exe").FileVersion.ToString
        End If
        ...
        If Not _strVersion_Update_Remote = "0" And Not _strVersion_Update_Lokal = _strVersion_Update_Remote Then
            ...
            If System.IO.File.Exists(_strApp_Pfad + "\RS_AP.Update.exe.alt") Then
                System.IO.File.Delete(_strApp_Pfad + "\RS_AP.Update.exe.alt")
            End If
            'Sicherungkopie erstellen
            If System.IO.File.Exists(_strApp_Pfad + "\RS_AP.Update.exe") Then
                System.IO.File.Move(_strApp_Pfad + "\RS_AP.Update.exe", _strApp_Pfad + "\RS_AP.Update.exe.alt")
            End If

            FileCopy(_strUpdate_Pfad + "\RS_AP.Update.exe", _
                     _strApp_Pfad + "\RS_AP.Update.exe")
        End If

        'Prüfen Version von KdMails.net()
        _strVersion_Remote = "0"

        If System.IO.File.Exists(_strUpdate_Pfad + "\RS_AP.exe") Then
            _strVersion_Remote = FileVersionInfo.GetVersionInfo(_strUpdate_Pfad + "\RS_AP.exe").FileVersion.ToString
        End If
        ...
        If Not _strVersion_Remote = "0" And Not _strVersion_Lokal = _strVersion_Remote Then
            ...
            If MsgBox("Es steht ein Update zur Verfügung!!!" & vbNewLine & _
                      ...
                      vbNewLine & _
                      "Wollen Sie dies verwenden?", MsgBoxStyle.YesNo, "RS_AP Auto Update") = vbYes Then

                Dim strCommand As String = ""

                strCommand = " /RDatei=""" & _strUpdate_Pfad & "\RS_AP.exe""" & _
                           " /LDatei=""" & _strApp_Pfad & "\RS_AP.exe""" & _
                           " /RVersion=""" & _strVersion_Remote & """" & _
                           " /LVersion=""" & _strVersion_Lokal & """"

                Shell(_strApp_Pfad + "\RS_AP.Update.exe" + strCommand, AppWinStyle.NormalFocus, False)
